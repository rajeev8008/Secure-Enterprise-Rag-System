...
Use short paragraphs and simple lists when they improve readability. Do not repeat source metadata in the answer."""
...
def _messages(question: str, chunks: list[RetrievedChunk]) -> list[tuple[str, str]]:
    context = "\n\n".join(
...
def generate_grounded_answer(question: str, chunks: list[RetrievedChunk], llm: Any) -> GenerationResult:
    response = llm.invoke(_messages(question, chunks))
